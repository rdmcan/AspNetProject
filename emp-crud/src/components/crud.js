...
const Crud = () => {
  ...
  const getData = async () => {
    try {
      const result = await axios.get("https://localhost:7085/api/Employee");
      setData(result.data);
    } catch (error) {
      console.log(error);
    }
  ...
  const handleEdit = async (id) => {
    handleOpen();
    const result = await axios.get(`https://localhost:7085/api/Employee/${id}`);
    setEditName(result.data.name);
    setEditAge(result.data.age);
    setEditIsActive(result.data.isActive); // 0, 1
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Do you want to delete this employee") === true) {
      const url = `https://localhost:7085/api/Employee/${id}`;
      try {
